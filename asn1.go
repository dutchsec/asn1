...
func encodeIdentifier(node *RawValue) ([]byte, error) {
	if node.Tag.Class > 0x03 {
		return nil, syntaxError("invalid class value: %d", node.Tag.Class)
	}

	identifier := []byte{0x00}

	// Class (bits 7 and 6) + primitive/constructed (1 bit) + tag (5 bits)
	identifier[0] += byte((node.Tag.Class & 0x03) << 6)
	...
	// Tag (bits 4 to 0)
	if node.Tag.Value <= 30 {
		identifier[0] += byte(0x1f & node.Tag.Value)
	} else {
		identifier[0] += 0x1f
		identifier = append(identifier, encodeMultiByteTag(node.Tag.Value)...)
	}
	return identifier, nil
}

func encodeMultiByteTag(tag ASNValue) []byte {
	...
	for i := range buf {
		...
		buf[i] = byte((uint(tag) & mask) >> shift)
...
func DecodeRawValue(reader io.Reader) (*RawValue, error) {
	tag, constructed, err := decodeIdentifier(reader)
	if err != nil {
		return nil, err
	}
	...
	raw := RawValue{tag, constructed, indefinite, content}
	return &raw, nil
}
...
func readEoc(reader io.Reader) error {
	for {
		...
		if tag.Class == 0 && tag.Value == 0 && indefinite == false && length == 0 {
			break
		}
...
func decodeIdentifier(reader io.Reader) (tag ASNTag, constructed bool, err error) {
	b, err := readByte(reader)
	if err != nil {
		return
	}
	...
	if value == 0x1f {
		// Tag is encoded in one or more following bytes
		value, err = decodeMultiByteTag(reader)
		if err != nil {
			return
		}
	}
